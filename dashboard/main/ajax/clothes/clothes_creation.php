<?php
    ...
    if($file_name)
    {
        $sql = "INSERT INTO product_clothes (`product_name`,`product_code`,`product_price`,`product_desc`) 
                    VALUES ('$p_name', '$p_code', '$p_price', '$p_desc')";
        ...
        if($result)
        {
            ...
            if(!file_exists('../../product_images/'.$insert_id.'/'))
            {
                mkdir('../../product_images/'.$insert_id.'/', 0777, true);
            }

            $path = '../../product_images/'.$insert_id.'/';
